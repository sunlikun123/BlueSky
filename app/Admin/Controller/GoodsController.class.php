...
class GoodsController extends AuthController {
    protected function _initialize(){
		...
		$news=$rs->where($map)->limit($Page->firstRow.','.$Page->listRows)->order('create_at desc')->relation(true)->select();
...
//        $join1 = "".C('DB_PREFIX').'admin as b on a.news_auto =b.admin_id';
	...
	public function books_runadd(){
		if (!IS_AJAX){
			$this->error('提交方式不正确',U('goods_list'),0);
		}
		$news=M('goods');
		//单图上传控制
		...
		$news->add($sl_data);
		$this->success('商品添加成功,返回列表页',U('goods_list'),1);
	}

	public function goods_edit(){
		$n_id = I('n_id');
		if (empty($n_id)){
			$this->error('参数错误',U('goods_list'),0);
		}else{
			$news_list=M('Goods')->where(array('n_id'=>I('n_id')))->find();
			/*
			 * 多图字符串转换成数组
			 */
			...
			$nav = new \Org\Util\Leftnav;
			$menu_next=M('goods_cate')->order('sort') -> select();
			$arr = $nav::menu_n($menu_next);
	...
	public function books_runedit(){
		...
		if($rst!==false){
			$this->success('商品修改成功,返回列表页',U('goods_list'),1);
		}else{
			$this->error('商品修改失败',U('goods_list'),0);
		}
	}

	public function books_del(){
		$p=I('p');
		$rst=M('goods')->where(array('n_id'=>I('n_id')))->setField('news_back',1);//转入回收站
		if($rst){
			$this->success('文章已转入回收站',U('goods_list',array('p' => $p)),1);
		}else{
			$this -> error("删除文章失败！",U('goods_list',array('p'=>$p)),0);
		}
	}
        
        public function goods_add_kucun(){
            $id = I('n_id');
            $this->assign('n_id',$id);
            $this->display();
	}
		...
		$books=M('goods');
                ...
                $log->add($data);
		if($rst!==false){
			$this->success('库存修改成功,返回列表页',U('goods_list'),1);
		}else{
			$this->error('库存修改失败',U('goods_list'),0);
		}
	}
        
        public function goods_reduce_kucun(){
            $id = I('n_id');
            $this->assign('n_id',$id);
            $this->display();
	}
        
        public function books_run_reducekucun(){
		if (!IS_AJAX){
			$this->error('提交方式不正确',U('goods_list'),0);
		}
                $id = I("n_id");
                $kucun = I("kucun");
		$books=M('goods');
                $number = $books->where('n_id='.$id)->field('books_num')->find();
                if ($number['books_num'] >= $kucun){
                    $rst = $books->where('n_id='.$id)->setDec('books_num',$kucun);  //设置库存减少
                }else{
                    $this->error('库存不足',U('goods_list'),0);
                }
                // 增加记录
                $log = M('goodslog');
                $data = array(
	...
	public function news_state(){
		$id=I('x');
		$status=M('goods')->where(array('n_id'=>$id))->getField('news_open');//判断当前状态情况
		if($status==1){
			$statedata = array('news_open'=>0);
			$auth_group=M('goods')->where(array('n_id'=>$id))->setField($statedata);
			$this->success('下架',1,1);
		}else{
	...
	public function news_back_open(){
		$p=I('p');
		$rst=M('goods')->where(array('n_id'=>I('n_id')))->setField('news_back',0);//转入正常
		if($rst!==false){
