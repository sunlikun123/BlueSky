...
class GoodscateController  extends AuthController {
    ...
    public function cate_list(){
		...
		$admin_rule=M('goods_cate')->order('sort')->select();
	...
	public function admin_rule_state(){
		$id=I('x');
		$statusone=M('goods_cate')->where(array('id'=>$id))->getField('status');//判断当前状态情况
		if($statusone==1){
			$statedata = array('status'=>0);
			$auth_group=M('goods_cate')->where(array('id'=>$id))->setField($statedata);
			clear_cache();
	...
	public function cate_edit(){
		//全部规则
		$nav = new \Org\Util\Leftnav;
		$admin_rule_all=M('goods_cate')->order('sort')->select();
		$arr = $nav::rule($admin_rule_all);
		$this->assign('cate',$arr);
		//待编辑规则
		$admin_rule=M('goods_cate')->where(array('id'=>I('id')))->find();
	...
	public function cate_del(){
        //TODO 自动删除子权限
		$rst=M('goods_cate')->where(array('id'=>I('id')))->delete();
        if($rst!==false){
			clear_cache();
