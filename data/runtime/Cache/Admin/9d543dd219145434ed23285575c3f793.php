						...
						<div class="table-responsive">
							<table class="table table-striped table-bordered table-hover" id="dynamic-table">
								...
								<tr>
									...
									<th class="hidden-sm hidden-xs">借书时间</th>
									<th class="hidden-xs">数量</th>
									<th class="hidden-xs">是否归还</th>
									<th class="hidden-xs">返还时间</th>
								...
								<?php if(is_array($news)): foreach($news as $key=>$v): ?><tr>
										...
										<td class="hidden-sm hidden-xs"><?php echo ($v["member_list_username"]); ?></td>
										<td class="hidden-xs"><?php echo (date('Y-m-d',$v["borrow_time"])); ?></td>
										<td class="hidden-xs">1</td>
										<td class="hidden-xs">
											<?php if($v['isback'] == 1): ?><a class="red state-btn" href="" data-id="" title="已归还">
													<div id="zt<?php echo ($v["n_id"]); ?>"><button class="btn btn-minier btn-yellow">已归还</button></div>
												</a>
												<?php else: ?>
												<a class="red state-btn" href="" data-id="" title="未归还">
													<div id="zt<?php echo ($v["n_id"]); ?>"><button class="btn btn-minier btn-danger">未归还</button></div>
												</a><?php endif; ?>
										</td>
										<td class="hidden-xs">
                                                                                    <?php if($v['isback'] == '1' ): echo (date('Y-m-d',$v["back_time"])); ?>
                                                                                            <?php else: ?>
                                                                                            未归还<?php endif; ?>
                                                                                </td>
										<td>
											<div class="hidden-sm hidden-xs action-buttons">
<!--												<a class="green" href="<?php echo U('books_edit',array('n_id'=>$v['n_id']));?>" title="修改">
													<i class="ace-icon fa fa-pencil bigger-130"></i>
												</a>-->
                                                                                                <?php if($v['isback'] == 0): ?><a class="green confirm-rst-url-btn" data-info="你确定要设置已归还吗？" href="<?php echo U('back',array('id'=>$v['id'],'bookid'=>$v['bookid']));?>" title="归还">
													<i class="ace-icon fa fa-check bigger-130"></i>
												</a><?php endif; ?>
<!--                                                                                                <a class="blue" href="<?php echo U('books_add_kucun',array('n_id'=>$v['n_id']));?>" title="增加库存">
